...
function UploadForm() {
  ...
  const [v, setV] = React.useState({ child: '', age: '', parent: '', email: '', phone: '' });
  ...
  const submit = (ev) => {
    ...
    reader.onload = () => {
      fetch(SHEET_ENDPOINT, {
        ...
        body: JSON.stringify({
          kindVorname: v.child, kindAlter: v.age, elternName: v.parent,
        ...
        {sent ? (
          <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-md)', padding: 'var(--space-12)', textAlign: 'center' }}>
            ...
            <div style={{ marginTop: 22 }}>
              <Button variant="outline" onClick={() => { setSent(false); setV({ child: '', age: '', parent: '', email: '', phone: '' }); setFile(null); setPreview(null); setConsentData(false); setConsentPub(false); }}>Weiteres Bild einreichen</Button>
            </div>
          </div>
        ) : (
          <form onSubmit={submit} noValidate className="form-card" style={{ background: 'var(--surface-card)', border: '1.5px solid #EEC803', borderRadius: 'var(--radius-xl)', boxShadow: '0 4px 12px 0 rgba(35,35,35,0.36)', padding: 'var(--space-8)', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 'var(--space-4)' }} className="f-row">
              <Input label="Vorname des Kindes" placeholder="z. B. Mia" value={v.child} onChange={set('child')} error={errors.child} />
              <div style={{ fontFamily: 'var(--font-sans)' }}>
                <label style={{ display: 'block', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-strong)', marginBottom: 6 }}>Alter</label>
                <div style={{ display: 'flex', alignItems: 'center', background: 'var(--surface-card)', border: `1.5px solid ${errors.age ? 'var(--red-500)' : 'var(--border-default)'}`, borderRadius: 'var(--radius-md)', padding: '0 8px 0 14px', minHeight: 46 }}>
                  <select value={v.age} onChange={set('age')} style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 'var(--text-md)', color: v.age ? 'var(--text-body)' : 'var(--text-faint)', padding: '12px 0', cursor: 'pointer', appearance: 'none' }}>
                    <option value="">Jahre</option>
                    {ageOptions.map((a) => <option key={a} value={a}>{a} Jahre</option>)}
                  </select>
                  <Ico name="ChevronDown" size={18} color="var(--text-muted)" />
                </div>
                {errors.age && <p style={{ margin: '6px 0 0', fontSize: 'var(--text-xs)', color: 'var(--red-500)', fontWeight: 600 }}>{errors.age}</p>}
              </div>
            </div>
